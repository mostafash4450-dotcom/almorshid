...
export default function ArticleEditor({ article, categories, databaseError }: Props) {
  ...
  const handleBlockquote = () => insertHTML('<blockquote>اكتب اقتباسك هنا</blockquote><p><br></p>');
  const handleTable = () => insertHTML(`
    <table>
  ...
  const handleSave = async (publishStatus: 'draft' | 'published') => {
    ...
    const articleData = {
      ...
      published_at: publishStatus === 'published' ? new Date().toISOString() : article?.published_at || null,
      updated_at: new Date().toISOString(),
    };

    let error;
    if (article?.id) {
      ({ error } = await supabase.from('articles').update(articleData).eq('id', article.id));
    } else {
  ...
  return (
    <div className="min-h-screen bg-gray-100" style={{ fontFamily: 'Cairo, sans-serif' }}>
      {/* Top bar */}
      <div className="bg-white border-b border-gray-200 sticky top-0 z-30" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        <div className="flex items-center justify-between px-6 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            {article?.slug && (
              <a href={`/articles/${article.slug}`} target="_blank" className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-800 border border-gray-200 px-3 py-1.5 rounded transition-colors">
                <Eye size={13} />
                معاينة
              </a>
            )}
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              <div className="flex items-center flex-wrap gap-1 px-4 py-2.5 border-b border-gray-100 bg-gray-50">
                ...
                <div className="flex items-center gap-0.5 border-l border-gray-200 pl-2 ml-1">
                  {toolbarBtn(() => execCmd('bold'), <Bold size={15} />, 'عريض')}
                  {toolbarBtn(() => execCmd('italic'), <Italic size={15} />, 'مائل')}
                  {toolbarBtn(() => execCmd('underline'), <Underline size={15} />, 'تحته خط')}
                </div>
                <div className="flex items-center gap-0.5 border-l border-gray-200 pl-2 ml-1">
                  {toolbarBtn(handleBulletList, <List size={15} />, 'قائمة نقطية')}
                  {toolbarBtn(handleNumberedList, <ListOrdered size={15} />, 'قائمة مرقمة')}
                </div>
                <div className="flex items-center gap-0.5 border-l border-gray-200 pl-2 ml-1">
              ...
              <div
                ...
                style={{ fontFamily: 'Tajawal, sans-serif', fontSize: '15px', lineHeight: 1.9 }}
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && editorRef.current) {
                    ...
                    if (sel && sel.rangeCount > 0) {
                      ...
                      if (parentEl && ['H1', 'H2', 'H3', 'BLOCKQUOTE'].includes(parentEl.tagName)) {
                        e.preventDefault();
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              <label className="block text-sm font-bold text-gray-700 mb-2">المقتطف</label>
              <textarea
                value={excerpt}
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              {showSeo && (
                <div className="px-5 pb-5 space-y-4 border-t border-gray-100">
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1.5 mt-4">Meta Title</label>
                    <input
                      type="text"
                      value={metaTitle}
                      ...
                      style={{ fontFamily: 'Cairo, sans-serif' }}
                    />
                    <div className="text-xs text-gray-400 mt-1">{metaTitle.length}/60 حرف</div>
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      style={{ fontFamily: 'Tajawal, sans-serif' }}
                    />
                    <div className="text-xs text-gray-400 mt-1">{metaDescription.length}/160 حرف</div>
                  </div>

                  {/* Preview */}
                  {(metaTitle || title) && (
                    <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
                      <p className="text-xs text-gray-400 mb-2">معاينة في جوجل:</p>
                      <div className="text-blue-600 text-sm font-medium" dir="rtl">{metaTitle || `${title} | المرشد المالي`}</div>
                      <div className="text-green-700 text-xs mt-0.5">almorshid-almali.com › articles › {slug}</div>
                      <div className="text-gray-500 text-xs mt-1" dir="rtl">{metaDescription || excerpt || 'لا يوجد وصف'}</div>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              <div className="p-4">
                <input
                  ...
                  style={{ fontFamily: 'Tajawal, sans-serif' }}
                />
                <p className="text-xs text-gray-400 mt-1.5">افصل الوسوم بفاصلة</p>
              </div>
            </div>
          </div>
        </div>
